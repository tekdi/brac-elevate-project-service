...
module.exports = class ProgramUsers extends Abstract {
	 ...
	 */
	async updateEntityProfile(req) {
		return new Promise(async (resolve, reject) => {
			try {
				const { userId, entityId, programId, updateData } = req.body

				const result = await programUsersHelper.updateEntityProfile(req.body, req.userDetails)
